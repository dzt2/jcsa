...
public class SBCIMutationGenerator extends AstMutationGenerator {
	...
	protected void generate_mutations(AstNode location, Collection<AstMutation> mutations) throws Exception {
		...
		if(!(statement instanceof AstBreakStatement))
			mutations.add(AstMutation.SBCI(statement, true));
		if(!(statement instanceof AstContinueStatement))
			mutations.add(AstMutation.SBCI(statement, false));
	}
