 ...
 * where expression is:
 * (1) logical or relational expression;
 * (2) condition in if, while, do, for statement;
 * (3) conditional expression's condition
 * @author yukimula
 *
 */
public class BTRPMutationGenerator extends AstMutationGenerator {
	...
	protected void collect_locations(AstNode location, Collection<AstNode> locations) throws Exception {
		...
		if(location instanceof AstIfStatement) {
			...
			condition = ((AstConditionalExpression) location).get_condition();
		}
	...
	protected void generate_mutations(AstNode location, Collection<AstMutation> mutations) throws Exception {
		...
		mutations.add(AstMutation.BTRP(expression, true));
		mutations.add(AstMutation.BTRP(expression, false));
	}
