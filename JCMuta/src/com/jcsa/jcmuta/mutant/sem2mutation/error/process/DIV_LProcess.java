...
public class DIV_LProcess extends StateErrorProcess {
	...
	protected void process_set_bool(SemanticAssertion source_assertion, CirNode target) throws Exception {
		...
		if(result != null) {
			if(result instanceof Long) {
				this.error_assertions.add(source_assertion.get_assertions().set_value(expression, ((Long) result).longValue()));
			}
			else {
				this.error_assertions.add(source_assertion.get_assertions().set_value(expression, ((Double) result).doubleValue()));
			}
		}
		else {
			this.error_assertions.add(source_assertion.get_assertions().mut_value(expression));
	...
	protected void process_dif_value(SemanticAssertion source_assertion, CirNode target) throws Exception {
		...
		Object lconstant = source_assertion.get_operand(1);
		Object rconstant = this.get_constant(roperand);
		Object difference = this.div(lconstant, rconstant);
		
		if(difference != null) {
			if(difference instanceof Long) {
				if(((Long) difference).longValue() != 0)
					this.error_assertions.add(source_assertion.get_assertions().diff_value(expression, ((Long) difference).longValue()));
			}
			else {
				if(((Double) difference).doubleValue() != 0)
					this.error_assertions.add(source_assertion.get_assertions().diff_value(expression, ((Double) difference).doubleValue()));
			}
		}
		else {
			this.error_assertions.add(source_assertion.get_assertions().mut_value(expression));
