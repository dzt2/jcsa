...
public class MOD_LProcess extends StateErrorProcess {
	...
	protected void process_mut_value(SemanticAssertion source_assertion, CirNode target) throws Exception {
		...
		if(rconstant != null) {
			if(this.is_one(rconstant) || this.is_neg_one(rconstant)) {
				/** equivalent mutation **/
			}
			else {
			...
			if(this.is_one(rconstant) || this.is_neg_one(rconstant)) {
				...
				if(result != null) {
					if(result instanceof Long) {
						...
						throw new IllegalArgumentException("Invalid MOD result");
					}
				}
				else {
					this.error_assertions.add(source_assertion.get_assertions().mut_value(expression));
				...
				if(result != null) {
					if(result instanceof Long) {
						if(((Long) result).longValue() != 0)
							this.error_assertions.add(source_assertion.get_assertions().
									diff_value(expression, ((Long) result).longValue()));
					}
					else {
						throw new IllegalArgumentException("Invalid MOD result");
					}
