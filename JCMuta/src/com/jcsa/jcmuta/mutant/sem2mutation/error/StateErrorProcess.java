...
public abstract class StateErrorProcess {
	 ...
	 * @throws Exception
	 */
	public List<StateError> process(StateError source_error, CirNode target) throws Exception {
		if(source_error == null)
			throw new IllegalArgumentException("Invalid source_error: null");
		else if(source_error.get_location() == null || source_error.number_of_assertions() == 0)
			return null;	/** when no more error can be caused from the source error as given **/
		else {
			this.const_assertions.clear();
			this.error_assertions.clear();
			
			for(SemanticAssertion source_assertion : source_error.get_assertions()) {
			...
			Map<CirNode, List<SemanticAssertion>> map = new HashMap<CirNode, List<SemanticAssertion>>();
			for(SemanticAssertion assertion : error_assertions) {
			...
			List<StateError> target_errors = new ArrayList<StateError>();
			for(CirNode location : map.keySet()) {
				StateError errors = source_error.get_graph().new_error(map.get(location));
				if(errors.number_of_assertions() > 0) target_errors.add(errors);
			}
			
			for(StateError target_error : target_errors) {
				source_error.get_graph().propagate(this.const_assertions, source_error, target_error);
			}
