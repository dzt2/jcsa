...
public class NOT_EProcess extends StateErrorProcess {
	...
	protected void process_dif_value(SemanticAssertion source_assertion, CirNode target) throws Exception {
		Object value = source_assertion.get_operand(1);
		value = this.cast_to_numeric_value(value);
		
		if(this.is_positive(value)) {
			this.process_inc_value(source_assertion, target);
		}
		else if(this.is_negative(value)) {
