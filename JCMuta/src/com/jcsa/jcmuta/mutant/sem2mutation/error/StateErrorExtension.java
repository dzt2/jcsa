...
/**
 * Used to build up the error set at one location in the program based on following rules:<br>
 * 	(1) <set_bool>	--> set_bool <not_value>	[when boolean context]<br>
 * 					--> <set_value> <dif_value>	[when numeric context]<br>
 * 	(2)	<not_value>	-->	not_value				[when boolean context]<br>
 * 					-->	<mut_value>				[when not-boolean context]<br>
 * 	(3)	<set_value>	-->	<set_bool>				[when boolean context]<br>
 * 					--> <dif_value> <neg|rsv>?	[when source is constant]<br>
 * 					--> set_value				[any case]<br>
 * 					--> <mut_value>				[otherwise]<br>
 * 	(4)	<dif_value>	-->	dif_value <inc|dec>		[any case]<br>
 * 					--> <set_value>				[when source is constant]<br>
 * 	(5)	<neg_value>	-->	neg_value <mut_value>	[when source is not const]<br>
 * 					--> neg_value <set_value>	[when source is constant]<br>	
 * 	(6)	<rsv_value>	-->	rsv_value <mut_value>
 * 	(7)	<mut_value>	-->	<not_value>				[when boolean context]<br>
 * 					--> mut_value				[otherwise in usage]<br>
 * 	(8)	<mut_refer>	-->	mut_refer <mut_value>	[when in usage point]<br>
 * @author yukimula
 *
 */
public class StateErrorExtension {
	...
	private static final StateErrorExtension extension = new StateErrorExtension();
	...
	/**
	 * complete the state error in the current set
	 * @param state_error
	 * @throws Exception
	 */
	public static void extend(StateError state_error) throws Exception {
		if(state_error == null)
			throw new IllegalArgumentException("Invalid state_error");
		else {
			extension.extend_all(state_error.get_assertions());
			state_error.set_assertions(extension.error_set);
		}
	}
