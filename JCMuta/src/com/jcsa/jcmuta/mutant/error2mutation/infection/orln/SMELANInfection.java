 ...
 *
 */
public class SMELANInfection extends OPRTInfection {
	...
	protected boolean partial_evaluate(CirExpression expression, CirExpression loperand, CirExpression roperand,
		...
		if(!(lconstant instanceof SymExpression)) {
			if(StateEvaluation.is_zero_number(lconstant)) {
				constraint = StateEvaluation.greater_eq(roperand, 0L);
		...
		if(!(rconstant instanceof SymExpression)) {
			if(StateEvaluation.is_zero_number(rconstant)) {
				constraint = StateEvaluation.smaller_eq(loperand, 0L);
				constraints = StateEvaluation.get_conjunctions();
				this.add_constraint(constraints, statement, constraint);
				output.put(graph.get_error_set().set_bool(expression, false), constraints);
			}
	...
	protected boolean symbolic_evaluate(CirExpression expression, CirExpression loperand, CirExpression roperand,
		...
		lcondition = StateEvaluation.logic_and(StateEvaluation.equal_with(loperand, 0L), 
				StateEvaluation.greater_eq(roperand, 0L));
		rcondition = StateEvaluation.logic_and(StateEvaluation.smaller_eq(loperand, 0L), 
