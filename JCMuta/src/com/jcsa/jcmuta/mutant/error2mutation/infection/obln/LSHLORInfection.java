 ...
 * loperand != 0 and roperand >= max_bitwise 	--> dif_numb(1)
 * loperand != 0								--> set_true
 * @author yukimula
 *
 */
public class LSHLORInfection extends OPRTInfection {
			...
			if(lconstant instanceof Long) {
				if(((Long) lconstant).longValue() == 0L) {
					...
					output.put(graph.get_error_set().
							set_bool(expression, true), StateEvaluation.get_conjunctions());
				}
			}
