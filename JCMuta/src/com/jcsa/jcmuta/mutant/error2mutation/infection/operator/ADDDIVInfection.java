 ...
 *
 */
public class ADDDIVInfection extends OPRTInfection {
	...
	protected boolean partial_evaluate(CirExpression expression, CirExpression loperand, CirExpression roperand,
		...
		if(!(rconstant instanceof SymExpression)) {
			if(rconstant instanceof Boolean) {
				...
				if(((Double) rconstant).doubleValue() == 0.0) {
					output.put(graph.get_error_set().failure(), 
							StateEvaluation.get_conjunctions());
					return true;
				}
				else if(((Double) rconstant).doubleValue() == 1.0) {
					output.put(graph.get_error_set().dif_numb(expression, -1L), 
							StateEvaluation.get_conjunctions());
					return true;
				}
		...
		/** unable to decide the error partially **/	return false;
	}
