 ...
 *
 */
public class GRTLANInfection extends OPRTInfection {
	...
	protected boolean partial_evaluate(CirExpression expression, CirExpression loperand, CirExpression roperand,
		...
		if(!(lconstant instanceof SymExpression)) {
			if(StateEvaluation.is_zero_number(lconstant)) {
				constraint = StateEvaluation.smaller_tn(roperand, 0L);
		...
		if(!(rconstant instanceof SymExpression)) {
			if(StateEvaluation.is_zero_number(rconstant)) {
				constraint = StateEvaluation.greater_tn(loperand, 0L);
				constraints = StateEvaluation.get_conjunctions();
				this.add_constraint(constraints, statement, constraint);
				output.put(graph.get_error_set().set_bool(expression, false), constraints);
			}
	...
	protected boolean symbolic_evaluate(CirExpression expression, CirExpression loperand, CirExpression roperand,
		...
		lcondition = StateEvaluation.logic_and(StateEvaluation.equal_with(loperand, 0L), StateEvaluation.smaller_tn(roperand, 0L));
		rcondition = StateEvaluation.logic_and(StateEvaluation.greater_tn(loperand, 0L), StateEvaluation.equal_with(roperand, 0L));
		constraints = StateEvaluation.get_disjunctions();
		this.add_constraint(constraints, statement, lcondition); this.add_constraint(constraints, statement, rcondition);
		output.put(graph.get_error_set().set_bool(expression, false), constraints);
		...
		constraints = StateEvaluation.get_conjunctions();
		this.add_constraint(constraints, statement, lcondition); this.add_constraint(constraints, statement, rcondition);
		this.add_constraint(constraints, statement, StateEvaluation.smaller_eq(loperand, roperand));
		output.put(graph.get_error_set().set_bool(expression, true), constraints);
