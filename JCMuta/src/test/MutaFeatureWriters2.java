...
public class MutaFeatureWriters2 {
	...
	protected static void testing(String name) throws Exception {
		...
		output_dependence_graph(project, output_directory);
	...
	/**
	 * [node] id context execution statement
	 * @param node
	 * @param writer
	 * @throws Exception
	 */
	private static void output_dependence_node(CDependNode node, FileWriter writer) throws Exception {
		...
		writer.write("\t");
		writer.write("" + node.get_instance().hashCode());
		writer.write("\t");
		writer.write(node.get_execution().toString());
		writer.write("\t");
		writer.write("cir#" + node.get_statement().get_node_id());
		writer.write("\n");
		
		for(CDependEdge edge : node.get_ou_edges()) 
			output_dependence_edge(edge, writer);
	}
	private static void output_dependence_graph(MutaProject project, File output_directory) throws Exception {
