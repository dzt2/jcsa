...
string newString(char * c, int t)
{
	...
	if(c!=NULL)
	{
		str.len = (t<=0) ? strlen(c) : t;
		str.c=(char *)malloc(sizeof(char)*(str.len+1));
		for(i=0; i<str.len; i++) 
			str.c[i]=c[i];
		str.c[str.len]='\0';
		return str;
	...
	else
	{
		str.len=t;
		str.c=(char *)malloc(sizeof(char)*(str.len+1));
		memset(str.c,(char)0,sizeof(char)*(t+1));
		str.sign = 1;
		return str;
	}
}

/* create string that concates the first and second */
...
uint32_t *MD4Digest(uint32_t *w, int len)
{
	...
	digest = malloc(sizeof(uint32_t)*4);
...
/** testing parts **/
/** maximal strng length is 64MB **/
#define MAX_STR_LENGTH	(1024 * 1024 * 64)
static int input_string_index;
static char input_string[MAX_STR_LENGTH];


static void append_input_string(char * argv)
{
	FILE * file; int i, j, k, length;
	file = fopen(argv, "rb");
	if(file == NULL)
	{
		length = strlen(argv);
		for(i = 0; i < length; i++)
		{
			input_string[input_string_index] = argv[i];
			input_string_index++;
		}
	}
	else
	{
		length = fread(input_string + input_string_index, sizeof(char), MAX_STR_LENGTH - input_string_index - 1, file);
		if(length >= 0)
		{
			input_string_index += length;
		}
		fclose(file);
	}
...
static void get_input_string(int argc, char * argv[])
{
	int i;
	input_string_index = 0;
	for(i = 1; i < argc; i++)
	{
		append_input_string(argv[i]);
	}
...
/**
 *	Usage: a.exe [file_path|text]*
 **/
int main(int argc, char *argv[])
{
	get_input_string(argc, argv);
	printf("%s\n", MD4(input_string, input_string_index));
	return 0;
}
