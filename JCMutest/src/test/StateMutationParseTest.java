...
import com.jcsa.jcmutest.backups.SecStateError;
import com.jcsa.jcmutest.backups.StateErrorPair;
import com.jcsa.jcmutest.backups.StateMutation;
...
public class StateMutationParseTest {
	...
	private static final String result_dir = "result/smu/";
	...
	private static void output_infection(StateMutation infection, FileWriter writer) throws Exception {
		...
		writer.write("+--------------------------------------------------------+\n");
		if(infection.has_beg_execution())
			writer.write("\tBeg-Statement: " + infection.
					get_beg_execution().get_statement().generate_code(true) + "\n");
		else
			writer.write("\tBeg-Statement: #None\n");
		if(infection.has_end_execution())
			writer.write("\tEnd-Statement: " + infection.
					get_end_execution().get_statement().generate_code(true) + "\n");
		else
			writer.write("\tEnd-Statement: #None\n");
		if(infection.has_pairs()) {
			int k = 0;
			for(StateErrorPair pair : infection.get_pairs()) {
				k++;
				writer.write("\tPair[" + k +"]:\n");
				/* optimized generation method */
				writer.write("\t\t" + pair.get_constraint().optimize(null).generate_code() + "\n");
				writer.write("\t\t" + pair.get_state_error().generate_code() + "\n");
				writer.write("\t\t" + pair.get_state_error().optimize(null).generate_code() + "\n");
				writer.write("\t\t{\n");
				for(SecStateError init_error : pair.get_state_error().extend(null)) {
	...
	protected static void testing(File cfile) throws Exception {
		...
		for(Mutant mutant : mspace.get_mutants()) {
			total++;
			try {
				StateMutation mutation = StateMutation.parse(code_file.get_cir_tree(), mutant);
				output_infection(mutation, writer);
			}
