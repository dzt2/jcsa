...
import com.jcsa.jcmutest.mutant.sec2mutant.mod.SecStateEdge;
import com.jcsa.jcmutest.mutant.sec2mutant.mod.SecStateGraph;
import com.jcsa.jcmutest.mutant.sec2mutant.mod.SecStateNode;
...
public class SecStateGraphTest {
	...
	private static final String result_dir = "result/inf/";
	...
	private static void write_graph(SecStateGraph graph, FileWriter writer) throws Exception {
		Mutant mutant = graph.get_infection().get_mutant();
		...
		writer.write("\tLocation@" + location.get_location().line_of() + ": ");
		...
		/* reach-point and infection pair */
		writer.write("\tReachPoint: " + graph.get_reach_node().get_unit().
				get_description().get_location().get_statement().generate_code(true) + "\n");
		SecStateNode reach_node = graph.get_reach_node();
		for(SecStateEdge edge : reach_node.get_ou_edges()) {
			writer.write("\t\t==> " + edge.get_target().get_unit().get_description().optimize().generate_code() + "\n");
			writer.write("\t\t\tUsing " + edge.get_constraint().get_description().optimize().generate_code() + "\n");
		}
	...
	protected static void testing(File cfile) throws Exception {
		...
		CDependGraph dgraph = CDependGraph.graph(translate(code_file.get_cir_tree()));
		...
		for(Mutant mutant : mspace.get_mutants()) {
			total++;
			SecInfection infection = null;
			try {
				infection = SecInfection.parse(code_file.get_cir_tree(), mutant);
			}
			catch(UnsupportedOperationException ex) {
				error++;
				write_mutant(mutant, writer2);
			}
			if(infection != null) {
				SecStateGraph sgraph = new SecStateGraph(dgraph, infection);
				write_graph(sgraph, writer);
			}
		}
