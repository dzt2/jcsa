...
public class SecStateGraphTest {
	...
	private static void write_state_graph(SecStateGraph graph, FileWriter writer) throws Exception {
		...
		if(graph.has_reach_node()) {
			...
			writer.write("{\n");
			writer.write("\tReach-Node: " + graph.get_reach_node().toString() + "\n");
			for(SecStateEdge edge : graph.get_reach_node().get_ou_edges()) {
				writer.write("\t==>\t" + edge.get_constraint(null).generate_code() + "\n");
				writer.write("\t\t" + edge.get_target().get_state_error().extend(null) + "\n");
			}
			writer.write("}\n");
		}
		writer.write("+------------------------------------------------------+\n\n");
	...
	protected static void testing(File cfile) throws Exception {
		...
		for(Mutant mutant : code_file.get_mutant_space().get_mutants()) {
			...
			SecStateGraph sgraph = new SecStateGraph(code_file.get_cir_tree(), mutant);
			sgraph.initialize(dgraph);
