...
public class OBANSadInfection extends SadInfection {
	...
	protected void get_infect(CirTree tree, AstMutation mutation, SadVertex reach_node) throws Exception {
		...
		if(statement != null) {
			if(operator == COperator.bit_and) {
				...
				switch(parameter) {
				case arith_add:	this.bitws_ior_to_arith_add(reach_node, statement, expression, loperand, roperand); break;
				case arith_sub:	this.bitws_ior_to_arith_sub(reach_node, statement, expression, loperand, roperand); break;
				case arith_mul:	this.bitws_ior_to_arith_mul(reach_node, statement, expression, loperand, roperand); break;
				case arith_div:	this.bitws_ior_to_arith_div(reach_node, statement, expression, loperand, roperand); break;
				case arith_mod:	this.bitws_ior_to_arith_mod(reach_node, statement, expression, loperand, roperand); break;
				default: throw new IllegalArgumentException("Invalid: " + parameter);
				}
			}
			else if(operator == COperator.bit_xor) {
				switch(parameter) {
				case arith_add:	this.bitws_xor_to_arith_add(reach_node, statement, expression, loperand, roperand); break;
				case arith_sub:	this.bitws_xor_to_arith_sub(reach_node, statement, expression, loperand, roperand); break;
				case arith_mul:	this.bitws_xor_to_arith_mul(reach_node, statement, expression, loperand, roperand); break;
				case arith_div:	this.bitws_xor_to_arith_div(reach_node, statement, expression, loperand, roperand); break;
				case arith_mod:	this.bitws_xor_to_arith_mod(reach_node, statement, expression, loperand, roperand); break;
				default: throw new IllegalArgumentException("Invalid: " + parameter);
				}
			}
			else if(operator == COperator.left_shift) {
				switch(parameter) {
				case arith_add:	this.bitws_lsh_to_arith_add(reach_node, statement, expression, loperand, roperand); break;
				case arith_sub:	this.bitws_lsh_to_arith_sub(reach_node, statement, expression, loperand, roperand); break;
				case arith_mul:	this.bitws_lsh_to_arith_mul(reach_node, statement, expression, loperand, roperand); break;
				case arith_div:	this.bitws_lsh_to_arith_div(reach_node, statement, expression, loperand, roperand); break;
				case arith_mod:	this.bitws_lsh_to_arith_mod(reach_node, statement, expression, loperand, roperand); break;
				default: throw new IllegalArgumentException("Invalid: " + parameter);
				}
			}
	 ...
	 * @throws Exception
	 */
	private void bitws_ior_to_arith_mod(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		...
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.
				set_expression(statement, expression, this.sad_expression(0));
		this.connect(reach_node, state_error, constraint);
	}
	/* x ^ y */
	/**
	 ...
	 * @throws Exception
	 */
	private void bitws_xor_to_arith_add(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadAssertion constraint, state_error;
		constraint = SadFactory.assert_condition(statement, not_equals(roperand, 0));
		state_error = SadFactory.set_expression(statement, expression, 
					this.arith_add(expression.get_data_type(), loperand, roperand));
		this.connect(reach_node, state_error, constraint);
	}
	 ...
	 * @throws Exception
	 */
	private void bitws_xor_to_arith_sub(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		condition = not_equals(bitws_and(expression.get_data_type(), loperand, roperand), roperand);
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(
				statement, expression, arith_sub(expression.get_data_type(), loperand, roperand));
		this.connect(reach_node, state_error, constraint);
	}
	/**
	 * [x == 0 || y == 0] --> set_expr(e, 0)
	 ...
	 * @throws Exception
	 */
	private void bitws_xor_to_arith_mul(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		
		condition = logic_ior(equal_with(loperand, 0), equal_with(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(statement, expression, this.sad_expression(0));
		this.connect(reach_node, state_error, constraint);
	 ...
	 * @throws Exception
	 */
	private void bitws_xor_to_arith_div(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		...
		condition = logic_and(equal_with(loperand, 0), not_equals(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(statement, expression, this.sad_expression(0));
		this.connect(reach_node, state_error, constraint);
	 ...
	 * @throws Exception
	 */
	private void bitws_xor_to_arith_mod(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		...
		condition = this.equal_with(loperand, this.arith_mul(expression.
				get_data_type(), this.any_value(CBasicTypeImpl.int_type), roperand));
		condition = this.logic_and(condition, this.not_equals(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(statement, expression, this.sad_expression(0));
		this.connect(reach_node, state_error, constraint);
		
		condition = this.not_equals(loperand, this.arith_mul(expression.
				get_data_type(), this.any_value(CBasicTypeImpl.int_type), roperand));
		condition = this.logic_and(condition, this.not_equals(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(statement, expression, 
	 ...
	 * @throws Exception
	 */
	private void bitws_lsh_to_arith_add(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		condition = logic_ior(not_equals(loperand, 0), not_equals(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
	 ...
	 * @throws Exception
	 */
	private void bitws_lsh_to_arith_sub(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadAssertion constraint, state_error;
	 ...
	 * @throws Exception
	 */
	private void bitws_lsh_to_arith_mul(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
		
		condition = this.logic_and(this.not_equals(loperand, 0), this.equal_with(roperand, 0));
		constraint = SadFactory.assert_condition(statement, condition);
		state_error = SadFactory.set_expression(statement, expression, this.sad_expression(0));
		this.connect(reach_node, state_error, constraint);
	 ...
	 * @throws Exception
	 */
	private void bitws_lsh_to_arith_div(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
	...
	/**
	 * [y == 0] --> trap()
	 * [y != 0 && x != k * y] --> set_expr(e, x % y)
	 * [y != 0 && x == k * y] --> set_expr(e, 0)
	 * @param reach_node
	 * @param statement
	 * @param expression
	 * @param loperand
	 ...
	 * @throws Exception
	 */
	private void bitws_lsh_to_arith_mod(SadVertex reach_node, CirStatement statement, 
			CirExpression expression, CirExpression loperand, CirExpression roperand) throws Exception {
		SadExpression condition; SadAssertion constraint, state_error;
