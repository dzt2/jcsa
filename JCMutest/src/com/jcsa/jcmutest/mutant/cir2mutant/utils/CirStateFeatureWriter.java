...
/**
 * It implements the feature writing over CirAbstractState(s).
 * 
 * @author yukimula
 *
 */
public class CirStateFeatureWriter {
	...
	private Map<CirAbstractState, Set<CirAbstractState>>	subsume_maps;
	 ...
	 * 			WORDS:	category$execution$location$loperand$roperand
	 * @throws Exception
	 */
	private String encode_test_token(Object token) throws Exception {
		if(token instanceof Mutant) {
			...
			String execution = this.encode_token(state.get_execution());
			String location = this.encode_token(state.get_location());
			String loperand = this.encode_token(state.get_loperand());
			String roperand = this.encode_token(state.get_roperand());
			return String.format("%s$%s$%s$%s$%s", category, execution, location, loperand, roperand);
		}
		else {
			return null;
		}
	...
	private void extend_subsume_at_state(CirAbstractState source, Object context, int distance) throws Exception {
		if(source == null || distance <= 0) { return; }		/* reach border */
			...
			if(!this.subsume_maps.containsKey(source)) {
				...
				try {
					Collection<CirAbstractState> targets = source.subsume(context);
	...
	private void extend_subsume_at_mutant(Mutant mutant, Object context, int max_distance) throws Exception {
		...
		if(cir_mutations != null && !cir_mutations.isEmpty()) {
			...
			for(CirMutation cir_mutation : cir_mutations) {
				if(!this.subsume_maps.containsKey(cir_mutation.get_r_state())) {
					this.subsume_maps.put(cir_mutation.get_r_state(), new HashSet<CirAbstractState>());
				}
	...
	private Iterable<Mutant> select_mutants_in(Object context) throws Exception {
		if(context instanceof CStatePath) {
			...
			for(Mutant mutant : this.source_cfile.get_mutant_space().get_mutants()) {
				for(CirMutation mutation : CirMutations.parse(mutant)) {
					CirExecution execution = mutation.get_execution();
			...
			for(CStateNode state_node : ((CStatePath) context).get_nodes()) {
				...
				if(exec_states.containsKey(execution)) {
					for(CirAbstractState state : exec_states.get(execution)) {
						this.extended_map.get(state).addAll(state.normalize(process).extend_all());
					}
				}
			}
		}
	 ...
	 * 	@param	[mutant_number, node_number, word_number]
	 * 	@throws Exception
	 */
	private int[] write_msg(Object context, String middile_name) throws Exception {
		...
		return new int[] { mid_set.size(), this.extended_map.size(), all_states.size() };
	}
	/**
	 * xxx.msh xxx.sym
	 * @param context
	...
	private void write_symb_features(Object context, String middle_name) throws Exception {
		...
		System.out.println(String.format("\t\t==> %d/%d mutants; %d (%d) states; %d symbol-nodes", 
					numbers[0], source_cfile.get_mutant_space().size(), 
					numbers[1], numbers[2], this.symbol_nodes.size()));
	}
