...
public class CirSetArithAddParser extends CirOperatorParser {

	@Override
	protected boolean to_assign() throws Exception {
		/** constraint: [x + y --> y] ==> [x != 0] **/
		CirAttribute constraint, init_error;
		SymbolExpression condition;
		condition = SymbolFactory.not_equals(loperand, 0);
		constraint = this.new_constraint(condition);
	...
	protected boolean arith_sub() throws Exception {
		...
		condition = SymbolFactory.not_equals(roperand, 0);
		constraint = this.new_constraint(condition);
	...
	protected boolean arith_div() throws Exception {
		...
		constraint = this.new_constraint(Boolean.TRUE);
	...
	protected boolean logic_ior() throws Exception {
		/** mutation [x + y; x || y] --> (either x or y is non_boolean) **/
		CirAttribute constraint, init_error; SymbolExpression muta_expression;
		List<SymbolExpression> conditions = new ArrayList<SymbolExpression>();
	...
	protected boolean greater_tn() throws Exception {
		...
		muta_expression = this.sym_expression(
				COperator.greater_tn, loperand, roperand);
		init_error = this.mut_expression(muta_expression);
	...
	protected boolean greater_eq() throws Exception {
		...
		constraint = this.get_constraint(Boolean.TRUE);
		muta_expression = this.sym_expression(
				COperator.greater_eq, loperand, roperand);
		init_error = this.mut_expression(muta_expression);
	...
	protected boolean smaller_tn() throws Exception {
		...
		constraint = this.get_constraint(Boolean.TRUE);
		muta_expression = this.sym_expression(
				COperator.smaller_tn, loperand, roperand);
		init_error = this.mut_expression(muta_expression);
	...
	protected boolean smaller_eq() throws Exception {
		...
		constraint = this.get_constraint(Boolean.TRUE);
		muta_expression = this.sym_expression(
				COperator.smaller_eq, loperand, roperand);
		init_error = this.mut_expression(muta_expression);
