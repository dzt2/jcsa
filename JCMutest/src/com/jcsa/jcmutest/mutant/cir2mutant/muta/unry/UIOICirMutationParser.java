...
public class UIOICirMutationParser extends CirMutationParser {
	...
	protected void generate_infections(CirTree cir_tree, CirStatement statement,
		...
		CirReferExpression reference = (CirReferExpression) this.get_cir_expression(cir_tree, mutation.get_location());
		...
		case insert_post_inc:
		{
			...
			infections.put(SymConditions.mut_stat(reference, muta_expression), constraint);
			break;
		}
		...
		case insert_prev_inc:
		{
			muta_expression = SymbolFactory.arith_add(reference.get_data_type(), reference, Integer.valueOf(1));
			infections.put(SymConditions.mut_stat(reference, muta_expression), constraint);
			infections.put(SymConditions.mut_expr(reference, muta_expression), constraint);
			break;
		}
