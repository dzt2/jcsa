...
public class ArithSubProcess extends SetOperatorProcess {
	...
	protected void arith_add() throws Exception {
		...
		if(this.compare) {
			...
			init_error = inc_expression(SymFactory.arith_mul(expression.
	...
	protected void arith_mod() throws Exception {
		...
		if(this.compare) {
			...
			constraints.add(get_constraint(SymFactory.smaller_tn(this.loperand, roperand)));
	...
	protected void bitws_xor() throws Exception {
		 ...
		 */
		SecConstraint constraint; SecDescription init_error;
		SymExpression expression, condition;
		
		expression = SymFactory.bitws_and(this.expression.
				get_data_type(), this.loperand, this.roperand);
		condition = SymFactory.not_equals(expression, this.roperand);
