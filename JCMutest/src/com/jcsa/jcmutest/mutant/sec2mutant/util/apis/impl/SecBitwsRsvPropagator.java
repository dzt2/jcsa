...
public class SecBitwsRsvPropagator extends SecExpressionPropagator {
	...
	protected void propagate_add_expression(SecAddExpressionError error) throws Exception {
		...
		case arith_mul:
		{
			...
			muta_expression = SymFactory.bitws_rsv(type, muta_operand);
		...
		case bit_and:
		{
			muta_operand = SymFactory.bitws_rsv(type, roperand);
			target_error = this.add_expression(COperator.bit_or, muta_operand);
			break;
		}
		case bit_or:
		{
			muta_operand = SymFactory.bitws_rsv(type, roperand);
			target_error = this.add_expression(COperator.bit_and, muta_operand);
			break;
		}
		case bit_xor:
		{
	...
	protected void propagate_uny_expression(SecUnyExpressionError error) throws Exception {
		...
		case negative:
		{
			muta_expression = SymFactory.arith_sub(type, operand, Integer.valueOf(1));
			target_error = this.set_expression(muta_expression);
			break;
		}
		case bit_not:
		...
		case logic_not:
		{
			...
			muta_expression = SymFactory.bitws_rsv(type, muta_expression);
			target_error = this.set_expression(muta_expression);
			break;
		}
		default: throw new IllegalArgumentException(operator.toString());
