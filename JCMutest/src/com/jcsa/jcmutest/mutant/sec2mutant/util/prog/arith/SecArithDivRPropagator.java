package com.jcsa.jcmutest.mutant.sec2mutant.util.prog.arith;
...
public class SecArithDivRPropagator extends SecExpressionPropagator {
	...
	protected void propagate_set_expression(SecSetExpressionError error) throws Exception {
		...
		SymExpression muta_expression = SymFactory.arith_div(type, loperand, roperand);
	...
	protected void propagate_uny_expression(SecUnyExpressionError error) throws Exception {
		...
		COperator operator = error.get_operator().get_operator();
		SecStateError target_error; SymExpression roperand, muta_expression;
		...
		case logic_not:
		{
			target_error = this.trap_statement();
			break;
		}
		default: throw new IllegalArgumentException(operator.toString());
		}
