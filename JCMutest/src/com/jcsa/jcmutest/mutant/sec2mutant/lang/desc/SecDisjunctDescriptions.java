...
public class SecDisjunctDescriptions extends SecDescriptions {
	...
	private SecDescription optimize_constraints(SymContexts contexts) throws Exception {
				...
				if(description instanceof SecConstraint) {
					...
					if(condition instanceof SymConstant) {
						if(((SymConstant) condition).get_bool()) {
							return description;	/* enforce as true in disjunctions */
						}
						else {
		...
		/* all of are false and get false in disjunctions */
		if(descriptions.isEmpty()) {
			return SecFactory.assert_constraint(this.get_location().get_statement(), Boolean.FALSE, true);
		}
		/* the unique constraint in the disjunctions */
		else if(descriptions.size() == 1) {
			return descriptions.values().iterator().next();
		}
		/* obtain the disjunctions for the descriptions being generated */
		else {
			return SecFactory.disjunct(this.get_location().get_statement(), descriptions.values());
		}
	}
