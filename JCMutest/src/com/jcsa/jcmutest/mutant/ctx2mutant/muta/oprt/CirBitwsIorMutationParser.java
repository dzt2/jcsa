...
public class CirBitwsIorMutationParser extends CirOperatorMutationParser {

	@Override
	protected boolean to_assign() throws Exception {
		/** (x |= y; x = y) :: (x != 0; x | y != y) --> (x = y) **/
		List<Object> conditions = new ArrayList<Object>();
		conditions.add(this.sym_expression(COperator.not_equals, this.get_loperand(), Integer.valueOf(0)));
		SymbolExpression operand = this.sym_expression(COperator.bit_or, this.get_loperand(), this.get_roperand());
		conditions.add(this.sym_expression(COperator.not_equals, operand, this.get_roperand()));
	...
	protected boolean bitws_and() throws Exception {
		...
		SymbolExpression condition = this.sym_expression(
				COperator.not_equals, this.get_loperand(), this.get_roperand());
