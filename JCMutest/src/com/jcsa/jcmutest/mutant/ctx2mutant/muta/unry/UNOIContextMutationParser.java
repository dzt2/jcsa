	...
	protected void parse_infection_set(AstCirNode location, AstMutation mutation) throws Exception {
		...
		if(mutation.get_operator() == MutaOperator.insert_arith_neg) {
			if(SymbolFactory.is_bool(orig_value)) {
				this.put_infection(this.eva_cond(Boolean.FALSE, false), 
								this.set_expr(orig_value, orig_value));
			}
			else {
				this.put_infection(
						this.eva_cond(SymbolFactory.not_equals(orig_value, 0), false), 
			...
			if(SymbolFactory.is_bool(orig_value)) {
				this.put_infection(
						this.eva_cond(SymbolFactory.sym_condition(orig_value, false), false),
			...
			if(SymbolFactory.is_bool(orig_value)) {
				...
				this.put_infection(
						this.eva_cond(SymbolFactory.smaller_tn(expression, 0), false), 
						this.set_expr(orig_value, SymbolFactory.arith_neg(orig_value)));
			}
		}
			...
			if(SymbolFactory.is_bool(orig_value)) {
				...
				this.put_infection(
						this.eva_cond(SymbolFactory.greater_tn(expression, 0), false), 
						this.set_expr(orig_value, SymbolFactory.arith_neg(orig_value)));
			}
		}
		else {
