...
public class TestInputExecution {
	...
	/** the template of command for copying instrumental result to specified file **/
	private static final String copy_instrument_result = "cp %s %s";
	 ...
	 * @param timeout the maximal seconds that is needed for running test 
	 * 		  or negative (or zero) when the time-out is not established.
	 * @param inst_result_file the file to preserve the instrumental analysis result.
	 * @param shell_file the file where the shell scripts of running tests are recorded.
	 * @throws Exception this process can take much time for running the test inputs!
	 */
	private static void generate_instrument_bash(File efile, Iterable<TestInput> inputs, File odir, 
			long timeout, File inst_result_file, File shell_file) throws Exception {
		FileWriter writer = new FileWriter(shell_file);
		writer.write(bash_shell_head);
		for(TestInput input : inputs) {
			writer.write(input.command(efile, odir, timeout) + "\n");
			writer.write(String.format(copy_instrument_result, 
					inst_result_file.getAbsolutePath(),
	 ...
	 * @throws Exception this process can take much time for running the test inputs!
	 */
	public static void do_instrument_testing(File efile, Iterable<TestInput> inputs, File odir, 
			long timeout, File inst_result_file, File shell_file, File cur_directory) throws Exception {
		/** 1. delete the original shell script file **/
		if(shell_file.exists()) {
			shell_file.delete();
			while(shell_file.exists());
		...
		/** 2. generate the shell script file **/
		TestInputExecution.generate_instrument_bash(efile, 
				inputs, odir, timeout, inst_result_file, shell_file);
