"""
This file implements the pattern mining module for recognizing interesting patterns from mutation in form
of RIP testing annotated with set of symbolic conditions required.
"""
...
class RIPMineContext:
	...
	def __init__(self, document: jcfeat.RIPDocument, exe_or_mut: bool, kill_or_not: bool,
				 stage: str, pass_threshold: float, min_support: int, min_confidence: float,
		...
		:param kill_or_not: True to take samples passing or not if False set
		:param stage: R_STAGE, I_STAGE, P_STAGE
		...
		"""
		self.patterns = jcfeat.RIPPatterns(document, stage, pass_threshold)
		self.solution = dict()
		# parameters
		self.exe_or_mut = exe_or_mut
		self.kill_or_not = kill_or_not
	...
	def is_kill_or_not(self):
		return self.kill_or_not
	...
	def estimate(self, pattern: jcfeat.RIPPattern):
		...
		if not(pattern in self.solution):
			total, support, confidence = pattern.estimate(self.exe_or_mut, self.kill_or_not)
...
class RIPPatternSpace:
	...
	def select_subsuming_patterns(patterns):
		...
		while len(remain_patterns) > 0:
			...
			if not (subsume_pattern is None):
				...
				minimal_pattern: jcfeat.RIPPattern
	...
	def get_subsuming_patterns(self):
		...
		:return:
		"""
		return RIPPatternSpace.select_subsuming_patterns(self.all_patterns)
	...
	def get_best_patterns(self, exe_or_mut: bool, uk_or_cc: bool):
		...
		for mutant, patterns in mutants_patterns.items():
			...
			if not (best_pattern is None):
				best_pattern: jcfeat.RIPPattern
...
class RIPPatternWriter:
	...
	def __write_pattern_summary__(self, pattern: jcfeat.RIPPattern):
		"""
		:param pattern:
		:return:
	...
	def __write_pattern_feature__(self, pattern: jcfeat.RIPPattern):
		"""
		:param pattern:
	...
	def __write_pattern_mutants__(self, pattern: jcfeat.RIPPattern):
		"""
		:param pattern:
	...
	def __write_pattern__(self, pattern: jcfeat.RIPPattern):
		self.output("#BEG\n")
	...
	def __write_evaluate_one__(self, index: int, pattern: jcfeat.RIPPattern):
		"""
		:param pattern:
...
class RIPFPTMiner:
	...
	def mine(self, context: RIPMineContext):
		...
		root_executions = context.get_classifier().select(
			context.get_document().get_executions(), context.is_kill_or_not())
...
class RIPDTTMiner:
	...
	def __input_context__(self, context: RIPMineContext):
		...
		for execution in self.context.get_document().get_executions():
			...
			total, support, confidence = self.context.get_classifier().estimate([sample], self.context.is_kill_or_not())
			if support > 0:
				self.Y.append(1)
			else:
				self.Y.append(0)
...
def evaluate_results(space: RIPPatternSpace, output_directory: str, name: str, exe_or_mut: bool, kill_or_not: bool):
	writer = RIPPatternWriter()
	writer.write_evaluate(space, os.path.join(output_directory, name + ".sum"))
	writer.write_matching(space, os.path.join(output_directory, name + ".bpt"), exe_or_mut, kill_or_not)
	writer.write_patterns(space.get_subsuming_patterns(), os.path.join(output_directory, name + ".mpt"))
	return


def get_rip_document(directory: str, file_name: str, output_directory: str):
	c_project = jcmuta.CProject(directory, file_name)
	document = jcfeat.RIPDocument(c_project, ".sft")
	if not(os.path.exists(output_directory)):
		os.mkdir(output_directory)
	return document


def do_frequent_mine(document: jcfeat.RIPDocument, exe_or_mut: bool, kill_or_not: bool,
					 stage: str, pass_threshold: float, min_support: int,
					 min_confidence: float, max_confidence: float, max_length: int, output_directory: str):
	miner = RIPFPTMiner()
	output_directory.strip()
	context = RIPMineContext(document, exe_or_mut, kill_or_not, stage, pass_threshold,
							 min_support, min_confidence, max_confidence, max_length)
	return miner.mine(context)


def do_decision_mine(document: jcfeat.RIPDocument, exe_or_mut: bool, kill_or_not: bool,
					 stage: str, pass_threshold: float, min_support: int,
					 min_confidence: float, max_confidence: float, max_length: int, output_directory: str):
	miner = RIPDTTMiner()
	context = RIPMineContext(document, exe_or_mut, kill_or_not, stage, pass_threshold,
...
def testing(inputs_directory: str, output_directory: str, model_name: str,
			exe_or_mut: bool, kill_or_not: bool, stage: str, pass_threshold: float,
			min_support: int, min_confidence: float, max_confidence: float,
			max_length: int, do_mining):
	"""
	...
	for file_name in os.listdir(inputs_directory):
		...
		space = do_mining(document=document, exe_or_mut=exe_or_mut, kill_or_not=kill_or_not, min_support=min_support,
						  min_confidence=min_confidence, stage=stage, pass_threshold=pass_threshold,
		...
		evaluate_results(space, output_directory, file_name, exe_or_mut, kill_or_not)
...
if __name__ == "__main__":
	...
	testing(prev_path, post_path, "decision_tree", True, False, jcfeat.P_STAGE, 0, 2, 0.75, 0.95, 8, do_decision_mine)
	testing(prev_path, post_path, "frequent_mine", True, False, jcfeat.P_STAGE, 0, 2, 0.70, 0.95, 8, do_frequent_mine)
