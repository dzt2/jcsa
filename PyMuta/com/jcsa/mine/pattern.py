...
class KillPredictionFPMine:
	"""
	It implements the frequent pattern mining to find good killable prediction rules.
	"""
	...
	def __mine__(self, parent: KillPredictionNode, features: list, used_tests):
		...
		self.caches[parent.get_rule()] = (length, support, confidence)
	...
	def __outs__(self, o_directory: str, file_name: str, c_document: jctest.CDocument, good_patterns, used_tests):
		...
		for c_annotation in  c_annotations:
			...
			m_pattern = self.middle.get_node([m_annotation.aid]).get_rule()
			...
			text = "C: {}\nE: {}\nS: {}\nU: {}\nV: {}\t[{}, {}%]".format(c_annotation.get_logic_type(),
																		 ...
																		 support, confidence)
			graph.node(key, text.replace('\"', '\'\'').strip())
		for c_annotation in c_annotations:
	...
	def mine(self, features, used_tests, is_reported, c_document, o_directory):
		...
		good_patterns = self.middle.filter_rules(self.caches.keys(), used_tests)
		if is_reported:
...
class KillPredictionDTMine:
	...
	def __out_decision_tree__(self, dc_tree: sktree.DecisionTreeClassifier, tree_file_path: str, c_document: jctest.CDocument):
		...
		for annotation in m_document.anot_space.get_annotations():
			...
			names.append(KillPredictionDTMine.__normalize_annotation__(annotation, c_document))
	...
	def __min_decision_path__(self, dc_tree: sktree.DecisionTreeClassifier):
		...
		for eid in range(0, len(self.middle.get_document().exec_space.get_executions())):
			...
			pattern = self.middle.get_node(eid_features).get_rule()
	...
	def mine(self, used_tests, c_document: jctest.CDocument, tree_file_path: str, is_reported: bool):
		...
		return self.middle.filter_rules(patterns, used_tests)
...
class KillPredictionWriter:
	...
	def __pat2str__(self, pattern: KillPredictionRule, used_tests=None):
		"""
	...
	def write_pattern_objects(self, middle: KillPredictionMiddle, patterns, file_path: str, used_tests, beg_line: str):
		"""
	...
	def write_pattern_metrics(self, middle: KillPredictionMiddle, patterns, file_path: str, used_tests, beg_line: str):
		"""
	...
	def write_mutant_patterns(self, middle: KillPredictionMiddle, patterns, file_path: str, used_tests, beg_line: str):
		"""
		:param middle:
		:param patterns:
		:param file_path:
	...
	def write_failed_mutation(self, middle: KillPredictionMiddle, patterns, file_path: str, used_tests, beg_line: str):
		"""
		:param middle:
		:param patterns:
		:param file_path:
	...
	def __mine_best_pattern__(self, document: jecode.MerDocument, mutant: jecode.MerMutant, miner: KillPredictionFPMine, max_tests_number: int):
		"""
		:param document:
		:param mutant:
		:param miner:
	...
	def write_mutant_clusters(self, inputs: KillPredictionInputs, file_path: str, beg_line: str, is_reported: bool, max_tests_number: int):
		"""
		:param inputs:
		:param file_path:
		:param beg_line:
		...
		pattern_mutants, pattern_tests, miner = dict(), dict(), KillPredictionFPMine(inputs)
		...
		for mutant in inputs.get_document().exec_space.get_mutants():
			...
			if best_pattern is None:
				...
				best_pattern: KillPredictionRule
...
def do_fpm_mining(c_document: jctest.CDocument, inputs: KillPredictionInputs,
	...
	fp_miner = KillPredictionFPMine(inputs)
	...
	writer = KillPredictionWriter(c_document, inputs)
...
def do_dtm_mining(c_document: jctest.CDocument, inputs: KillPredictionInputs,
				  o_directory: str, file_name: str, used_tests, is_reported: bool):
	"""
	:param c_document: 		the document of mutation testing project and its data source
	:param inputs: 			the input module to drive the pattern mining procedures
	...
	dt_miner = KillPredictionDTMine(inputs)
...
def do_fp_cluster(c_document: jctest.CDocument, inputs: KillPredictionInputs,
