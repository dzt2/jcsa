...
import com.jcsa.mine.feature2 as jcfeature
...
class RIPDTMiner:
	...
	def __input_context__(self, inputs: jcfeature.RIPMineInputs):
		...
		for sequence in inputs.get_document().get_sequences():
			...
			total, support, confidence = inputs.get_classifier().estimate([sequence], inputs.get_support_strategy())
			if support > 0:
				self.Y.append(1)
			else:
				self.Y.append(0)
...
def get_model_directory_name(model_name: str, sample_class, support_strategy):
	"""
	:param model_name:
	:param sample_class:
	:param support_strategy:
	:return:
	"""
	if sample_class is None:
		...
		sample_class = "seq"
	if support_strategy == jcfeature.unk_support_strategy:
		support_strategy = "unk"
	elif support_strategy == jcfeature.wcc_support_strategy:
		support_strategy = "wcc"
	elif support_strategy == jcfeature.scc_support_strategy:
		support_strategy = "scc"
	elif support_strategy == jcfeature.kid_support_strategy:
...
def do_mining(document: jctest.CDocument, output_directory: str, model_name: str,
			  sample_class, support_strategy, used_tests, max_length: int,
	...
	:param support_strategy:	UK_SUPPORT_CLASS, WC_SUPPORT_CLASS, SC_SUPPORT_CLASS
	...
	:return:
	"""
	model_name = get_model_directory_name(model_name, sample_class, support_strategy)
	print("\tDo Mining on", document.get_program().name, "for model of", model_name)
	# 1. create inputs of mining algorithm
	inputs = jcfeature.RIPMineInputs(document, used_tests, sample_class, support_strategy,
...
def do_mining_on_classes(document: jctest.CDocument, output_directory: str, model_name: str,
	...
	:return:
	"""
	do_mining(document, output_directory, model_name, jcfeature.MU_SAMPLE_CLASS, jcfeature.unk_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	do_mining(document, output_directory, model_name, jcfeature.MU_SAMPLE_CLASS, jcfeature.wcc_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	do_mining(document, output_directory, model_name, jcfeature.MU_SAMPLE_CLASS, jcfeature.scc_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	do_mining(document, output_directory, model_name, jcfeature.SQ_SAMPLE_CLASS, jcfeature.unk_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	do_mining(document, output_directory, model_name, jcfeature.SQ_SAMPLE_CLASS, jcfeature.wcc_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	do_mining(document, output_directory, model_name, jcfeature.SQ_SAMPLE_CLASS, jcfeature.scc_support_strategy,
			  used_tests, max_length, min_support, min_confidence, max_confidence, do_model_mining)
	return
