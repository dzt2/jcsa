"""
This file implements frequent pattern mining based on execution result.
"""
...
class MutationClassifier:
	"""
	It implements the classification on MutantExecutionLine or Mutant
	"""
	def __init__(self):
		self.cache = dict()		# from sample to (uc, ui, up, ki)
		return
	...
	def evaluate_one(self, sample):
		...
		if not(sample in self.cache):
			...
			if isinstance(sample, cmuta.Mutant):
				...
				for test_id in range(0, s_result.get_length()):
					if s_result.is_killed_by(test_id):
						ki += 1
					elif w_result.is_killed_by(test_id):
						up += 1
					elif c_result.is_killed_by(test_id):
						ui += 1
					else:
						uc += 1
			else:
...
class MutationPatternGenerator:
	...
	def __init__(self, line_or_mutant: bool, uk_or_cc: bool, min_support: int, max_precision: float, max_length: int):
		...
		:param classifier: used to classify the mutant or execution line
		"""
...
if __name__ == "__main__":
	...
	for file_name in os.listdir(root_path):
		...
		MutationPatternWriter.write_patterns(docs, god_patterns, classifier, os.path.join(post_path, file_name + ".gpt"))
		...
		MutationPatternWriter.write_best_patterns(min_patterns, classifier, os.path.join(post_path, file_name + ".bpt"), line_or_mutant, uk_or_cc)
