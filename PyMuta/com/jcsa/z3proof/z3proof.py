...
class SymbolToZ3Prover:
	...
	def __z3_ucheck__(self, constraint, assumptions):
		"""
		:param constraint:	the z3.Expression to denote the constraint being verified
	...
	def prove_mutant(self, mutant: jcmuta.Mutant):
		project = mutant.get_space().get_project()
		is_equal, state, result = False, None, self.neq_class
		if project.context_space.has_mutation(mutant):
			mutation = project.context_space.get_mutation(mutant)
			is_equal, state, result = self.prove_mutation(mutation)
		return is_equal, state, result

	def prove_project(self, project: jcmuta.CProject):
		"""
		:param project:
		...
		counter, total, steps = 0, len(project.muta_space.mutants), 3000

		beg_time = time.time()
		for mutant in project.muta_space.get_mutants():
			mutant: jcmuta.Mutant
			is_equal, state, result = self.prove_mutant(mutant)
			if counter % steps == 0:
				print("\t\tProceed[{}/{}]\t{}\t{}".format(counter, total, is_equal, self.class_name(result)))
			counter = counter + 1
			...
			if is_equal:
				...
				equal_number += 1
				if result == self.veq_class:
					veq_number += 1
				elif result == self.seq_class:
					seq_number += 1
				else:
					beq_number += 1
				output[mutant] = (state, result)
		end_time = time.time()
...
def test_symbol_z3_prover(project: jcmuta.CProject, tce_set, file_path: str):
	...
	mutant_state_class = prover.prove_project(project)
