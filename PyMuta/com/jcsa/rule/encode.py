""" This file defines the memory-reduced model for encoding features used in pattern mining. """
...
class C2MerDocumentParser:
	"""
	It implements the parsing from CDocument to MerDocument using a given strategy of annotation encoding
	"""

	def __init__(self, c_document: jctest.CDocument, e_directory: str):
		"""
		:param c_document: the source document to provide original data objects
		:param e_directory: the directory where the directory of encoded files are generated
		"""
		self.file_name = c_document.get_program().name
		self.c_document = c_document
		self.e_directory = os.path.join(e_directory, self.file_name)
		if not os.path.exists(self.e_directory):
			os.mkdir(self.e_directory)
		self.feature_dict = dict()	# mapping from word of SymCondition to its integer feature in encoded files
		return

	## file encoding module

	def __encode_mut__(self):
		"""
		:return: e_directory/file_name.mut for mutations
		"""
		with open(os.path.join(self.e_directory, self.file_name + ".mut"), 'w') as writer:
			for mutant in self.c_document.project.muta_space.get_mutants():
	...
	def __encode_tst__(self):
		"""
		:return: e_directory/file_name.tst for test cases
		"""
		with open(os.path.join(self.e_directory, self.file_name + ".tst"), 'w') as writer:
			for test in self.c_document.project.test_space.get_test_cases():
				test: jcmuta.TestCase
				writer.write("{}\n".format(test.get_test_id()))
		return

	def __encode_res__(self):
		"""
		:return: e_directory/file_name.res for mutation test results
		"""
		with open(os.path.join(self.e_directory, self.file_name + ".res"), 'w') as writer:
			for mutant in self.c_document.project.muta_space.get_mutants():
				mutant: jcmuta.Mutant
	...
	def __encode_sym__(self):
		"""
		:return: e_directory/file_name.sym for symbolic annotations and update mapping from their string to integer ID
		"""
		cid = 0
		self.feature_dict.clear()
		with open(os.path.join(self.e_directory, self.file_name + ".sym"), 'w') as writer:
			for condition in self.c_document.get_condition_space().get_conditions():
				writer.write("{}\n".format(str(condition)))
				self.feature_dict[condition] = cid
				cid += 1
		return

	def __encode_exc__(self, annotation_selector):
		"""
		:param annotation_selector: SymExecution --> Iter[SymCondition] to select right annotations from each line
		:return: e_directory/file_name.exc for symbolic execution with annotations in testing.
		"""
		with open(os.path.join(self.e_directory, self.file_name + ".exc"), 'w') as writer:
			for execution in self.c_document.get_execution_space().get_executions():
				## mid tid head
				...
				annotations = annotation_selector(execution)
				for annotation in annotations:
					cid = self.feature_dict[annotation]
					writer.write("\t{}".format(cid))
				writer.write("\n")
		return

	@staticmethod
	def encode(c_document: jctest.CDocument, e_directory: str, annotation_selector):
		"""
		:param c_document: the source document to provide original data objects
		:param e_directory: the directory where the directory of encoded files are generated
		:param annotation_selector: SymExecution --> Iter[SymCondition] to select right annotations from each line
		:return:
		"""
		parser = C2MerDocumentParser(c_document, e_directory)
		parser.__encode_mut__()
		parser.__encode_tst__()
		parser.__encode_res__()
		parser.__encode_sym__()
		parser.__encode_exc__(annotation_selector)
		return

	## annotation selectors

	@staticmethod
	def all_selector(execution: jctest.SymExecution):
		"""
		:param execution:
		:return: all the annotations used in execution state nodes
		"""
		annotations = set()
	...
	@staticmethod
	def pre_selector(execution: jctest.SymExecution):
		"""
		:param execution:
		:return: only to select pre_condition + mid_condition annotation
		"""
		annotations = set()
		for state in execution.get_states():
			category = state.get_attribute().get_category()
			if category == "pre_condition" or category == "mid_condition":
				for annotation in state.get_annotations():
					annotations.add(annotation)
		return annotations

	@staticmethod
	def fir_selector(execution: jctest.SymExecution):
		annotations, nex_states = C2MerDocumentParser.pre_selector(execution), list()
		for state in execution.get_states():
			category = state.get_attribute().get_category()
			if category == "nex_condition":
				nex_states.append(state)
		if len(nex_states) > 0:
			state = nex_states[0]
			for annotation in state.get_annotations():
				annotations.add(annotation)
	...
	@staticmethod
	def las_selector(execution: jctest.SymExecution):
		"""
		:param execution:
		:return:
		"""
...
def encode_c_documents(prev_path: str, post_path: str, exec_postfix: str, annotation_selector):
	"""
	...
	for file_name in os.listdir(prev_path):
		...
		C2MerDocumentParser.encode(c_document, post_path, annotation_selector)
...
def main(prev_path: str, post_path: str, exec_postfix: str, annotation_selector):
	encode_c_documents(prev_path, post_path, exec_postfix, annotation_selector)
...
if __name__ == "__main__":
	...
	exit_code = main(prev_directory, post_directory, ".stp", C2MerDocumentParser.all_selector)
	exit(exit_code)
